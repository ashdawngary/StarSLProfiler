...
max_num = export_to_lam(num_max)
min_num = export_to_lam(num_min)
add1_num = pythoniclam(lambda x: num(trynum(x)["value"] + 1))
sub1_num = pythoniclam(lambda x: num(trynum(x)["value"] + 1))
abs_num = pythoniclam(lambda x: num(abs(trynum(x)["value"])))
...
modu_num = pythoniclam(lambda x, y: num(trynum(x)["value"] % trynum(y)["value"]))
sqr_num = pythoniclam(lambda x: num(trynum(x)["value"] ** 2))
sqrt_num = pythoniclam(lambda x: num(sqrt(trynum(x)["value"])))
num_tostr = pythoniclam(lambda x: string(consisestr(trynum(x)["value"])))
num_even = pythoniclam(lambda x: boolean(trynum(x)["value"] % 2 == 0))
num_odd = pythoniclam(lambda x: boolean(trynum(x)["value"] % 2 == 1))
num_zerohuh = pythoniclam(lambda x: boolean(trynum(x)["value"] == 0))
num_neghuh = pythoniclam(lambda x: boolean(trynum(x)["value"] < 0))
num_poshuh = pythoniclam(lambda x: boolean(trynum(x)["value"] > 0))